...
import { fetchSource，StreamContentType } from 'fetch-source'
const ctrl = new AbortController()
fetchSource('/api/sse', {
    // method: 'POST',
    // headers: {
    ...
    signal: ctrl.signal,
    onopen(response) {
        if (response.ok && response.headers.get('content-type') === StreamContentType) {
            return;
        } else if (response.status >= 400 && response.status < 500 && response.status !== 429) {
          const errorMessage = statusMap[response.status] || `服务器错误 (${response.status})`;
